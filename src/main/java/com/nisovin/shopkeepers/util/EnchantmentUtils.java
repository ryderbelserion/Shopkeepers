...
public class EnchantmentUtils {
	...
	public static final class EnchantmentEntry {
		...
		public String toString() {
			...
			builder.append("EnchantmentEntry [enchantment=");
		...
		public boolean equals(Object obj) {
			...
			if (!(obj instanceof EnchantmentEntry)) return false;
			EnchantmentEntry other = (EnchantmentEntry) obj;
	...
	// The level parsing is quite lenient: If the specified level is invalid, we fallback to a valid level.
	// TODO Add an option to toggle this behavior to this method.
	public static EnchantmentEntry parseEnchantmentWithLevel(String input) {
		if (input == null) return null;
		...
		if (lastSpace != -1) {
			String levelString = input.substring(lastSpace + 1);
			...
			if (level != null) {
				...
				// Could not parse the level. Maybe the section behind the last space is part of the enchantment name.
				// We therefore initially keep the input as is. If it is not part of the enchantment name but actually
				// an invalid level, parsing the enchantment will initially fail. We then try to parse the enchantment
				// again without this portion of the input.
			}
		...
		if (level == null || level == Integer.MIN_VALUE) {
			...
			// Note: We allow players to specify enchantment levels that are outside the bounds used in vanilla
			// Minecraft, because they work fine for the most part and may therefore be used on servers.
			if (level < MIN_LEVEL) {
				level = MIN_LEVEL;
	...
	/**
	 * Checks if the given enchantment level is within the {@link Enchantment#getStartLevel() lower} and
