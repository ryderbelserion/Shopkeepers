...
/**
 * Produces the {@link AbstractShopObject#getId() shop object ids} that are used by {@link BlockShopObject}s.
 * <p>
 * All shop objects that represent their shopkeepers by blocks are required to use these common shop object ids. This
 * ensures that lookups of the shopkeeper for a given block are quick and do not require to individually query all
 * registered {@link BlockShopObjectType}s.
 */
public final class BlockShopObjectIds {

	/**
	 * Gets the object id that is or would be used by a {@link BlockShopObject} that represents the shopkeeper by the
	 * given {@link Block}.
	 * 
	 * 
	 * @param block
	...
	/**
	 * Gets the object id that is or would be used by a {@link BlockShopObject} that represents the shopkeeper by the
	 * block at the specified coordinates.
	 * 
	 * @param worldName
	...
	public static Object getObjectId(String worldName, int blockX, int blockY, int blockZ) {
		// Note: We could also use the block itself as id. However, by using a separate type of object that is under our
		// control, we can slightly improve the performance of lookups by block coordinates by reusing a shared
		// BlockLocation instance. Otherwise, we would need to retrieve a new Block object for every lookup. This
		// optimization is for example useful when we handle the high frequency BlockPhysicsEvent.
	...
	// Note: We do not need to reset the shared block location after use. Its attributes are overwritten with subsequent
	// use, and it does not hold on to any costly resources.
