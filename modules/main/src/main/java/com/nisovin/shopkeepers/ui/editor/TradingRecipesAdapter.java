...
/**
 * Different types of merchants differ in how they internally represent their offers. This {@link TradingRecipesAdapter}
 * converts between the merchant's offers and the {@link TradingRecipeDraft}s that are visualized and edited in the
 * editor.
 * <p>
 ...
 * partially setup but still invalid trading recipe drafts, for example to fill the editor with suggestions or examples
 * for the editing player on possible trading recipes.
 * <p>
 * Once the player exists the editor, {@link #updateTradingRecipes(Player, List)} is invoked with the list of the
 * potentially edited trading recipe drafts. This method is then responsible to update the merchant's offers based on
 * these trading recipe drafts, and to report back whether and how many offers have changed.
 */
public interface TradingRecipesAdapter {
	 ...
	 * <p>
	 * Implementations of this method can assume that the given trading recipe drafts and their items are no longer used
	 * by the editor afterwards. Implementations are therefore allowed to reuse these objects.
	 * 
	 * @param player
