 ...
 * after the task finishes its current execution. The next execution of the task is then responsible to perform the work
 * that has accumulated in the meantime.
 * <p>
 * If not noted otherwise, all methods are meant to be called from the main thread. The task cannot be interacted with
 * from within executions, such as attempting to trigger another execution.
 * <p>
 * A possible use case is an IO task that writes to a specific file, but of which only one execution is allowed to take
...
public abstract class SingletonTask {
	...
	private enum State {
		 ...
		 * <p>
		 * For an asynchronous execution this state may get set asynchronously. In this case, checking for it has to be
		 * guarded by the execution lock.
		 */
		EXECUTING,
		/**
	...
	// Note: This lock is not provided to the outside of this class, because it is not suited for the coordination with
	// other tasks. Each task execution completes with the execution of its synchronous callback. So instead, other
	// tasks can use #awaitExecutions() to wait for any current and pending executions to complete.
	...
	private Runnable internalCallback = null; // Run immediately, possibly asynchronously
	private Runnable internalSyncCallback = null;
	// Flag that indicates whether the task should execute again after its current execution finishes:
	...
	private void validateMainThreadAndNotWithinExecution() {
		...
		// Note: If there is an async execution in progress, we may observe state PENDING or state EXECUTING. Calling
		// this method from the main thread is allowed in this case.
	}

	/**
	 * Performs shutdown logic for this task.
	 * <p>
	 * This will wait for any currently active execution and any pending requested execution to finish.
	 * <p>
	 * This method cannot be called from within an execution, i.e. from within {@link #prepare()}, {@link #execute()},
	 * or {@link #syncCallback()}.
	 */
	public final void shutdown() {
		this.validateMainThreadAndNotWithinExecution();
		// Wait for any current async execution to finish. This may also trigger and wait for any other pending
		// execution.
	 ...
	 * <p>
	 * If an asynchronous execution is already in progress, the task will be executed again once the current execution
	 * completes.
	 * <p>
	 * During plugin disable (after the plugin has already been marked as {@link Plugin#isEnabled() disabled}),
	 * asynchronous executions are not supported. Any execution requests will then take place immediately.
	 * <p>
	 * This method cannot be called from within an execution, i.e. from within {@link #prepare()}, {@link #execute()},
	 * or {@link #syncCallback()}.
	 */
	 ...
	 * <p>
	 * If an asynchronous execution is already in progress, this method waits for it to finish before performing the
	 * requested execution. This method blocks until the requested execution completes.
	 * <p>
	 * This method cannot be called from within an execution, i.e. from within {@link #prepare()}, {@link #execute()},
	 * or {@link #syncCallback()}.
	 */
	 ...
	 * is made. Otherwise, frequent requests to start the task could continually abort the task so that it is never able
	 * to complete.
	 * <p>
	 * Otherwise, if the async execution has already started, this will wait for it to finish. The sync callback will
	 * then need to be manually run by us in order to complete the previous execution.
	 * <p>
	 * If there is another execution pending, the sync callback of the previous execution will trigger another sync
	 * execution. This method will also wait for that second execution to finish.
	 */
	public final void awaitExecutions() {
		...
		// If an async execution is currently executing, acquiring the lock will wait for it to finish.
		// If the async execution has not taken the lock yet, then its is either still pending to be started by the
		// Scheduler on the main thread (which we are currently blocking), or it has already been started but not
		// acquired the lock yet. In the latter case there may or may not already exist a worker thread for it, and it
		// may or may not have checked its cancellation state yet.
		synchronized (executionLock) {
			if (state == State.PENDING) {
				// The task has not yet started its execution / taken the lock yet.
				// We cancel it and manually run it here. Note: We cancel this within the locked section, because the
				// cancellation is only required (and effective) if the task has not started executing yet. Reliably
				// checking for this state requires the lock.
				// However, a worker thread may already exist for it and the task may already have checked its
				// cancellation state. This is why the async task has to check its cancellation state again within the
				// locked section in order to reliably detect it.
				asyncTask.cancel();
			} else {
		...
		runAgainSync = true;
		if (!hasExecuted) {
			// We reset the asyncTask variable here to indicate that we are synchronously executing (required for the
			// validation of runTask(boolean)). The asyncExecution variable still reflects the original state.
			asyncTask = null;
			this.executeTask(null);
			// Assertion: This will also invoke the immediate callback, which invokes the sync callback, which executes
			// immediately as well because we are already in the main thread here.
			assert internalSyncCallback == null;
		} else {
			// Else: The execution took place asynchronously, so the sync callback must still be pending.
			assert internalSyncCallback != null;
			internalSyncCallback.run();
			// This will reset the execution state, so that the pending task for the sync callback can detect that it
			// has already been run here.
		}
		// Note: The sync callback of the previous execution will also trigger and immediately execute any pending
		// execution.
		runAgainSync = false; // Reset

		// At this point, any previously executing and pending executions have been completed, and any state related to
		// them has been reset.
	}

	private void runTask(boolean async) {
		this.validateMainThreadAndNotWithinExecution();
		// During plugin disable, only synchronous executions are allowed, because the Bukkit Scheduler does no longer
		// accept new tasks. This has to be ensured by the caller (i.e. other methods of this class).
		Validate.State.isTrue(!async || plugin.isEnabled(), "Cannot execute asynchronously during or after plugin disable!");
		...
		if (asyncTask != null) {
			...
			if (async) {
				...
				// Wait for the current async execution to finish and then immediately execute again:
				this.awaitExecutions();
				return;
			}
		...
		assert internalSyncCallback == null;
		assert !runAgain;
		// Other execution information and statistics get incrementally overwritten during the next execution.
		...
		internalCallback = () -> {
			...
			// This gets omitted here if the plugin got disabled during an asynchronous execution. In this case the
			// sync callback is manually run when the shutdown is handled on the main thread.
			// Assertion: The syncCallback is still the one associated with this callback's execution. Preparation of
			// any subsequent execution either waits for the previous execution and this callback to complete, or the
			// async task for this execution and callback are cancelled and invoked manually.
			// Also note: If this callback is run from the main thread, the sync callback is run immediately.
			SchedulerUtils.runOnMainThreadOrOmit(plugin, internalSyncCallback);
		};

		// Sync callback: Gets run on the main thread after the execution has completed. This is run prior to any
		...
		if (async) {
			...
			// If the async task gets cancelled later, a new execution (and new asyncTask) may get prepared before this
			// task is actually run. We therefore cannot retrieve the executing task from this class' asyncTask
			// variable. Instead we capture the task here.
	...
	// If the async task got cancelled and another execution has already been started, this may not match the current
	// value of this class' asyncTask variable.
	private void executeTask(BukkitTask asyncTask) {
		if (asyncTask != null) {
			// Asynchronous execution:
			...
			synchronized (executionLock) {
				...
				// cancelled).
				// The async task has already checked the cancellation state. However, there is a race condition between
				// checking the state and the actual execution. Checking it here again within the locked section avoids
				// this race condition.
	 ...
	 * <p>
	 * Note that if a request is made to immediately execute the task, any currently pending asynchronous execution may
	 * actually take place synchronously on the main thread but still be considered 'asynchronous' by this method.
	 * 
	 ...
	 * <p>
	 * The lock is only acquired for asynchronous executions. For synchronous executions this returns <code>0</code>.
	 * <p>
	 * This value might be useful for debugging purposes and should usually be relatively small (less than
	...
	/**
	 * Gets the delay between the task {@link #prepare() preparation} and the actual start of the previous execution.
	 * <p>
	 * Synchronous executions take place immediately after preparation. Consequently, this value will usually only be
	 * interesting for {@link #isAsyncExecution() asynchronous executions}.
	 * <p>
	 * For asynchronous executions this includes the time it took to acquire the {@link #getLockAcquireDuration()
	 * execution lock}, but which should usually be very small and therefore not noticeable in this statistic.
	 * 
	...
	/**
	 * Gets the total duration of the previous execution, from preparation to execution completion, including all delays
	 * caused by the scheduling of any asynchronous task.
	 * <p>
