...
public class PlayerShopsLimit {
	...
	private static void registerMaxShopsPermissions() {
		// Note: These permissions are registered only once, and then never unregistered again until the server is fully
		// reloaded or restarted. This is analogous to how the declared permissions of plugins are registered only once
		// when the plugins are loaded, and not every time a plugin is disabled and enabled again.
		// Also, Bukkit doesn't even properly support dynamically unregistering permissions: For example, there is no
		// way to update the cached default permissions via the API after having unregistered a permission (however,
		// this isn't really required for our permissions anyways). And we would have to manually update all players and
		// other Permissibles that might have these permissions attached to them.
		// However, these permissions are pretty lightweight, so there is no harm caused by keeping them registered
		// until the server restarts, even if the list of max shops permissions has changed after a config reload.
	 ...
	 * <p>
	 * This is calculated based on the configured default maximum shops limit, and the player's specific maximum shops
	 * permissions.
	 * 
	 * @param player
