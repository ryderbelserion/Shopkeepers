 ...
 * <p>
 * By default, we reuse the merchant's previous offers and only replace those that changed. The previous offers and the
 * new offers derived from the trading recipe drafts are matched by their index.
 *
 * @param <O>
...
public abstract class DefaultTradingRecipesAdapter<O> implements TradingRecipesAdapter {
	 ...
	 * <p>
	 * The order of these trading recipe drafts is expected to match the order of the corresponding offers returned by
	 * {@link #getOffers()}.
	 */
	@Override
	public abstract List<TradingRecipeDraft> getTradingRecipes();

	// TODO Maybe keep track of which TradingRecipeDrafts have actually been edited (i.e. clicked at in the editor or
	// marked as 'dirty' by some other mean), and then avoid creating offers and comparing items for them.
	 ...
	 * <p>
	 * The order of these offers is expected to match the order of the corresponding trading recipe drafts returned by
	 * {@link #getTradingRecipes()}.
	 * 
	 ...
	 * <p>
	 * The given trading recipe draft is {@link TradingRecipeDraft#isValid() valid}. However, this method may implement
	 * additional verifications and return <code>null</code> if it considers the trading recipe draft to be invalid
	 * according to these checks: If this is the case, no offer is added to the merchant for the given trading recipe
	 * draft, any previous offer is removed, and {@link #handleInvalidTradingRecipe(Player, TradingRecipeDraft)} is
	 * invoked.
	 * <p>
	 * Implementations of this method can assume that the given trading recipe draft and its items are no longer used by
	 * the editor afterwards. Implementations are therefore allowed to reuse these objects.
	 * 
	 * @param recipe
	 ...
	 * <p>
	 * This method is called when the new offers {@link #createOffer(TradingRecipeDraft) created} based on the trading
	 * recipe drafts from the editor are compared with the merchant's previous offers. This method serves two purposes:
	 * <ul>
	 * <li>It is used to detect whether the offers have changed.
	 * <li>It allows to decide which of these two offers shall be further used by the merchant: If the given offers are
	 * considered equal by this method, the previous offer is reused when building the list of new offers for the
	 * merchant. Otherwise, the new offer replaces the previous one.
	 * </ul>
	 * <p>
	 * By default, this method compares the offers based on {@link Object#equals(Object)}. However, this method may be
	 * overridden if there are cases in which the offers may not be fully equal, but equal enough that the old offer is
	 * meant to be reused nevertheless and not be replaced by the new offer. This situation can for example arise if the
	 * editor does not represent all aspects of the merchant's previous offers.
	 * 
	 * @param oldOffer
	 ...
	 * {@link TradingRecipeDraft#isValid() invalid} trading recipe draft, as well as for every trading recipe draft that
	 * is considered invalid by {@link #createOffer(TradingRecipeDraft)}.
	 * 
	 * @param player
	 *            the editing player
	 * @param invalidRecipe
