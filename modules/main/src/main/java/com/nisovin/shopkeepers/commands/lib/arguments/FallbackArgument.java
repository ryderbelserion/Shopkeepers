 ...
 * <p>
 * Parsing may fail because the current input argument is either invalid, or because the command argument is optional
 * and the current input argument is meant to bind to the next command argument instead. To deal with this ambiguity,
 * the fallback doesn't get evaluated immediately, but only after checking if the following command arguments are able
 * to parse the remaining input arguments. This gets indicated to the parsing command by throwing a
...
public abstract class FallbackArgument<T> extends CommandArgument<T> {
	...
	/**
	 * Parses a fallback value for this argument and stores it in the {@link CommandContext} with the argument's name as
	 * key.
	 * <p>
	 ...
	 */
	public abstract T parseFallback(CommandInput input, CommandContext context, ArgumentsReader argsReader,
