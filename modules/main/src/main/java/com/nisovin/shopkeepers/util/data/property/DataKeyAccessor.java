...
/**
 * A {@link DataAccessor} that uses a {@link DataSerializer} to save and load its value from the data at a specific data
 * key.
 *
 * @param <T>
...
public class DataKeyAccessor<T> implements DataAccessor<T> {
	...
	/**
	 * Sets the {@link Predicate} that decides if the data accessed by this {@link DataKeyAccessor} shall be considered
	 * empty and therefore treated as if it is missing.
	 * <p>
	 * The predicate is only evaluated for non-<code>null</code> data: <code>null</code> is always considered missing
	 * data.
	 * <p>
	 * Emptiness is usually defined for a specific data type, but the predicate can be evaluated for data of arbitrary
	 * type. The predicate is expected to evaluate to <code>false</code> for any data that does not match its emptiness
	 * definition.
	 * 
	 * @param <A>
		...
		if (value == null) {
			...
			if (serialized == null) {
				throw new RuntimeException("Serializer of type " + serializer.getClass().getName()
						+ " serialized value to null: " + value);
			}
	...
	/**
	 * Uses the {@link #getSerializer() DataSerializer} to reconstruct the value from the data stored in the given
	 * {@link DataContainer} at the {@link #getDataKey() data key} of this {@link DataKeyAccessor}.
	 * 
	 * @param dataContainer
	 *            the data container not <code>null</code>
	 * @return the loaded value, not <code>null</code>
	 * @throws MissingDataException
	 *             if the data for the value is missing, or satisfies the {@link #emptyDataPredicate(Predicate) empty
	 *             data predicate}
	...
	/**
	 * Creates a {@link MissingDataException} that is used when the data for this {@link DataKeyAccessor} is missing.
	 * 
	...
	/**
	 * Gets the raw data that is stored in the given data container at this data accessor's {@link #getDataKey() data
	 * key}.
	 * 
	 * @param dataContainer
	 *            the data container, not <code>null</code>
	...
	/**
	 * Sets the raw data in the given data container at this data accessor's {@link #getDataKey() data key}.
	 * 
	 * @param dataContainer
	 *            the data container, not <code>null</code>
	 * @param data
	 ...
	 */
	private final void setData(DataContainer dataContainer, Object data) {
		assert dataContainer != null;
