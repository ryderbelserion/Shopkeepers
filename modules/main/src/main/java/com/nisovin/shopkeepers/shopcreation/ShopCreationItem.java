...
import com.nisovin.shopkeepers.util.bukkit.BooleanPersistentDataType;
...
public final class ShopCreationItem {
	...
	public static boolean hasTag(@ReadOnly @Nullable ItemStack itemStack) {
		...
		return dataContainer.has(KEY_SHOP_CREATION_ITEM, BooleanPersistentDataType.INSTANCE);
	}
	...
	public static boolean addTag(@ReadWrite @Nullable ItemStack itemStack) {
		...
		PersistentDataContainer dataContainer = meta.getPersistentDataContainer();
		// TODO Use has(NamespacedKey) without the data type validation once we depend on 1.20.4+?
		if (dataContainer.has(KEY_SHOP_CREATION_ITEM, BooleanPersistentDataType.INSTANCE)) {
			return false;
		}

		dataContainer.set(KEY_SHOP_CREATION_ITEM, BooleanPersistentDataType.INSTANCE, true);
