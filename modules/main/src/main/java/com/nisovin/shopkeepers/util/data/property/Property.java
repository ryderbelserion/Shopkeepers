 ...
 * <p>
 * While {@link DataSerializer} provides low-level means to serialize and deserialize data types, a {@link Property} may
 * handle additional higher level aspects, such as its association with one or more specific data keys, the definition
 * of default values, and the validation of values specific to the property's usage context and semantic.
 *
 * @param <T>
...
public interface Property<T> extends DataAccessor<T> {
	 ...
	 * <p>
	 * The name should be suitable for users to identify the property in error, log, or other feedback messages.
	 * Properties that store their data under a single data key may for example use this data key as their name.
	 * 
	...
	/**
	 * Gets the {@link DataAccessor} that is used to save and load values for this property.
	 * <p>
	 * The returned data accessor bypasses all the value validations and other logic that is provided by this property
	 * on top of the data accessor's own logic.
	 * 
	 * @return the data accessor, not <code>null</code>
	 */
	 ...
	 * <p>
	 * This assumes that any potentially necessary data migrations have already been applied to the given data.
	 * <p>
	 * If the data is missing for this property, the property may either return a fallback value (which might be
	 * <code>null</code>), or throw a {@link MissingDataException}. The exact behavior depends on the concrete property
	 * implementation. To always take the {@link #getDefaultValue() default value} into account, one can use
	 * {@link #loadOrDefault(DataContainer)}.
	 * 
	 * @param dataContainer
	 *            the data container not <code>null</code>
	 * @return the loaded value, or an optional fallback value (which can be <code>null</code>) if there is no data for
	 *         this property
	 * @throws MissingDataException
	 *             if the data for the value is missing and this property does not provide a fallback value
	 ...
	 * {@link #getDefaultValue() default value} if the data for the property is missing. If the data is missing and this
	 * property has no {@link #hasDefaultValue() valid default value}, this throws a {@link MissingDataException} just
	 * like {@link #load(DataContainer)}.
	 * 
	 * @param dataContainer
	 *            the data container not <code>null</code>
	 * @return the loaded value, can be <code>null</code> if there is no data for this property and the default value is
	 *         <code>null</code>
	 * @throws MissingDataException
